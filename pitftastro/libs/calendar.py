...
logger = logging.getLogger("pitftastro.libs.calendar")
...
class Calendar(threading.Thread):
    ...
    def sort_by_date(obj: dict):
        ...
        """
        if obj.get("start"):
            if isinstance(obj["start"], date) and not isinstance(
                obj["start"], datetime
            ):
                return datetime.combine(
                    obj["start"], datetime.min.time(), local_timezone
                )
            ...
            if isinstance(obj["due"], date) and not isinstance(
                obj["due"], datetime
            ):
                return datetime.combine(
                    obj["due"], datetime.min.time(), local_timezone
                )
    ...
    def get_events_from_webcal(self, new_events, url):
        ...
        try:
            ...
            for event in timeline:
                ...
                new_events.append({"start": start, "summary": summary})
    ...
    def get_events_from_caldav(
        self, new_events, new_tasks, url, username, password
    ):
        """
        ...
        for cal in calendars:
            calendar_events = cal.date_search(
                start=datetime.today(),
                end=datetime.today() + timedelta(days=7),
                expand=True,
            )
            for event in calendar_events:
                start = self.standardize_date(
            ...
            for todo in todos:
                try:
                    due = self.standardize_date(
                        todo.vobject_instance.vtodo.due.value
                    )
                ...
                new_tasks.append({"due": due, "summary": summary})
    ...
    def get_latest_events(self):
        ...
        for connection in CALENDAR_URLS:
            if str(connection["type"]).lower() == "webcal":
                ...
                logger.error(
                    "calendar type not recognized: {0}".format(
                        str(connection["type"])
                    )
                )
    ...
    def tasks_as_string(self):
        ...
        for obj in self.tasks:
            ...
            if obj["due"]:
                text += (
                    "  - Due: " + self.humanized_datetime(obj["due"]) + "\n"
                )
    ...
    def humanized_datetime(self, dt: datetime):
        ...
        try:
            ...
            obj = dt
        if (
            isinstance(obj, date) and not isinstance(obj, datetime)
        ) or obj.date() > datetime.today().date():
