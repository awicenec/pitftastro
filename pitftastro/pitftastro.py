...
logger = logging.getLogger("pitftastro")
...
class PiTFTAstro:
    ...
    def __init__(self):
        ...
        self.mq = posix_ipc.MessageQueue(
            "/pitftastro_ipc", flags=posix_ipc.O_CREAT
        )
        self.mq.block = False
        ...
        logger.info(
            "PiTFT Size: {0}x{1}".format(
                self.framebuffer.size[0], self.framebuffer.size[1]
            )
        )
                ...
                for event in self.pitft_touchscreen.get_event():
                    if (
                        ...
                        #     self.current_app.swipe("up")
                            ...
                            position = get_pixels_from_coordinates(
                                self.framebuffer, (self.touch_x, self.touch_y)
                            )
