...
logger = logging.getLogger("pitftastro.apps.dashboard")
...
class App(AbstractApp):
    ...
    def reload(self):
        ...
        if len(self.calendar.events) > 0:
            start = self.calendar.standardize_date(
                self.calendar.events[0]["start"]
            )
        ...
        if len(self.calendar.tasks) > 0:
            ...
            if self.calendar.tasks[0].get("due"):
                text = " - Due: " + self.calendar.humanized_datetime(
                    self.calendar.tasks[0]["due"]
                )
                current_line += self.text(
                    text,
                    font_size=24,
                    position=(5, 200),
                    color="yellow",
            ...
            self.text(
                text,
                font_size=24,
                position=(5, 200 + current_line * 30),
                max_lines=2,
            )
        else:
